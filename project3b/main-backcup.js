...
var plotData = function(month,day,year,dayName,isSummedTogether) {
	...
	//show summary totals
	$(".banging .num").increment({coef: 1, speed:40, limit:banging});
	$(".horn .num").increment({coef: 1, speed:40, limit:horn});
	$(".carMusic .num").increment({coef: 1, speed:40, limit:carMusic});
	$(".engine .num").increment({coef: 1, speed:40, limit:engine});
	$(".party .num").increment({coef: 1, speed:40, limit:party});
	$(".talking .num").increment({coef: 1, speed:40, limit:talking});
	$(".tv .num").increment({coef: 1, speed:40, limit:tv});
...
for(var i = 0; i<12; i++) {
	...
	console.log("days length: " + daysInYear.length);*/
		vizSvg = d3.select(".viz").append("svg").style("width","1260");
		vizSvg.on('mouseout', function(d) {
  				 	$(".dateToShow").text("Complaints in 2016");
  				 })
		//var monthGroup;
		var xTotal = 0;
	for(var i = 0; i<12; i++) {
		...
		monthGroup.selectAll("rect")
	       ...
	       .attr("x",function(d,i){ return i*3;})
	         ...
	         .attr("height",function(d){ return d*15}); //console.log("d: " + d); 

	         monthGroup.attr("class",i);
